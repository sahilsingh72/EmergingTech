...
<body class="hold-transition sidebar-mini layout-fixed">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @if($row->main_bill_url)
                                                                <a href="{{ route('trainer.travel.preview', ['path' => $row->main_bill]) }}" target="_blank"
                                                                    class="btn btn-sm btn-primary mb-1 w-full">
                                                                        View Main Bill
                                                                    </a>
                                                            @else
                                                            ...
                                                            @if($row->has_return)
                                                                ...
                                                                @if($row->return_bill_url)
                                                                    <a href="{{ route('preview.file', ['path' => $row->return_bill_file]) }}" target="_blank"
                                                        ...
                                                        <td>
                                                            ...
                                                            @if(auth()->user()->role_id == 8)
                                                                ...
                                                                <form action="{{ route('trainerTravel.updateTrainingDate', $row->id) }}" 
                                                        ...
                                                        <td>
                                                            ...
                                                            @if(in_array(Auth::user()->role_id, [8]))
                                                                @if($row->status == 'Pending')
                                                                    ...
                                                                    <form action="{{ route('trainerTravel.approve', $row->id) }}" method="POST" class="d-inline">
                                                                        @csrf
                                                                        <button class="btn btn-success btn-sm mt-1 w-full">Approve</button>
                                                                    </form>

                                                                    <button class="btn btn-danger btn-sm mt-1 w-full" data-toggle="modal" data-target="#rejectModal{{ $row->id }}">
                                                                        Reject
                                                                    </button>
                                                                    ...
                                                                    <form action="{{ route('trainerTravel.revert', $row->id) }}" method="POST">
                                                                        @csrf
                                                                ...
                                                                <div class="modal fade" id="rejectModal{{ $row->id }}">
                                                                    <div class="modal-dialog">
                                                                        <form action="{{ route('trainerTravel.reject', $row->id) }}" method="POST">
                                                                            @csrf
    ...
    <!-- Edit Travel Modal -->
    <div id="editModal" class="content-wrapper fixed inset-0 hidden z-50 bg-black bg-opacity-50 flex items-center justify-center px-2">
        <div class="bg-white rounded-lg shadow-lg max-w-5xl w-full max-h-[80vh] overflow-y-auto p-6">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="main_from" id="edit_main_from" class="w-full border p-2 rounded" required>
                </div>
                <div>
                    <label class="block font-semibold mb-1">To</label>
                    <input type="text" name="main_to" id="edit_main_to" class="w-full border p-2 rounded" readonly>
                </div>
                <div>
                    <label class="block font-semibold mb-1">Date of Travel</label>
                    <input type="date" name="main_date" id="edit_main_date" class="w-full border p-2 rounded" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="main_amount" id="edit_main_amount" class="w-full border p-2 rounded" required>
                </div>
                <div>
            ...
            <div id="editReturnSection" class="border rounded p-4 mb-4 bg-gray-50 hidden">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="return_from" id="edit_return_from" class="w-full border p-2 rounded" readonly>
                    </div>
                    <div>
                        <label class="block font-semibold mb-1">Return To</label>
                        <input type="text" name="return_to" id="edit_return_to" class="w-full border p-2 rounded" placeholder="full address">
                    </div>
                    <div>
                        <label class="block font-semibold mb-1">Mode of Travel</label>
                    ...
                    <div>
                        <label class="block font-semibold mb-1">Amount (₹)</label>
                        <input type="number" name="return_amount" id="edit_return_amount" class="w-full border p-2 rounded" placeholder="Amount">
                    </div>
                    <div>
            ...
            <button type="button" id="editAddReturnBtn" class="mb-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded hidden">
                + Add Return Travel
            </button>

    
                <div class="mt-4 flex justify-end gap-2">
                    <button type="button" class="bg-gray-400 px-4 py-2 rounded" onclick="closeEditModal()">Cancel</button>
