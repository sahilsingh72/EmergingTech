...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <script>
        ...
        let billOptions = ["Generator", "Inauguration", "Misc"];
        let usedOptions = [];
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" 
                    name="amount[]"
                    class="amountInput border p-2 rounded w-full"
                    placeholder="Amount"
                    min="1"
                    required />
            </div>
            ...
            <div>
                ...
                <input type="file" accept="application/pdf" 
                    name="bill_file[]"
                    class="billFileInput border p-2 rounded w-full" required/>

                    <small class="text-gray-500 text-xs">
                        PDF only • Max size 5 MB
                    </small>

                    <small class="fileError text-red-600 text-xs hidden"></small>
            </div>
        </div>
    `;
            ...
            row.querySelector(".removeBtn").addEventListener("click", () => {
                ...
                function restoreOption(value) {
                    if (billOptions.includes(value)) {
                        usedOptions = usedOptions.filter(v => v !== value);
                    }
                }
                row.remove();
            ...
            select.addEventListener("change", (e) => {
                ...
                if (selected === "Misc") {
                    ...
                    // }
                    if (billOptions.includes(selected) && !usedOptions.includes(selected)) {
                        usedOptions.push(selected);
                    }
                }
...
document.addEventListener('change', function (e) {
    ...
    if (!e.target.classList.contains('billFileInput')) return;

    const file = e.target.files[0];
    const errorEl = e.target.parentElement.querySelector('.fileError');
    ...
    if (file.size > maxSize) {
        errorEl.innerText = 'File size must not exceed 5 MB.';
